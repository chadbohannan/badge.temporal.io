...
def check_links(
    pages: dict[str, str], wiki_root: str
) -> list[tuple[str, str, str]]:
    # `pages` keys are wiki-relative (e.g. "log.md", "components/x.md") because
    # they were loaded relative to the wiki dir. Resolve every link to an
    # absolute on-disk path against the absolute wiki root, then accept it if
    # it is a known page OR it exists anywhere in the checkout (which lets
    # pages cite ../sources/*.md and ../scripts/README.md by convention).
    # Build a set of absolute on-disk paths for every known page.
    abs_pages = {os.path.normpath(os.path.join(wiki_root, p)) for p in pages}
    broken: list[tuple[str, str, str]] = []
    for path, content in pages.items():
        source_abs = os.path.normpath(os.path.join(wiki_root, path))
        base_abs = os.path.dirname(source_abs)
        for m in LINK_RE.finditer(content):
            target = urllib.parse.unquote(m.group(1))
            if target.startswith(("http://", "https://")):
                continue
            resolved_abs = os.path.normpath(os.path.join(base_abs, target))
            if resolved_abs in abs_pages:
                continue
            if os.path.isfile(resolved_abs):  # e.g. ../sources/foo.md
                continue
            broken.append((path, target, os.path.relpath(resolved_abs, wiki_root)))
    return broken
...
def main() -> int:
    ...
    # Resolve all links against the absolute wiki dir; links that escape it
    # (../sources/*.md, ../scripts/*.md) are then checked on disk against the
    # repo checkout that contains the wiki dir.
    ...
    broken = check_links(pages, wiki_root)
    print("\n=== Broken links ===")
    if broken:
