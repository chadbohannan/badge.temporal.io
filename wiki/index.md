...
- [Vectortank: Native Wireframe Vector Game](components/vectortank.md) — the Battlezone-style native C++ game that replaced a MicroPython prototype after benchmarking found a real `oled::drawLine()` overflow bug, and how its FPS-walker controls work around the badge's 1-stick/4-button budget
- [Helgrind: Norse-Themed Action-RPG](components/helgrind.md) — the badge's second native game on the `helgrind_game` branch: a classic-era top-down RPG over a fixed 8×8 room grid (one room per ambient-LED pixel), a strictly linear seven-artifact chain, three weapons, and a `badge_helgrind` NVS save — design brief plus the first playable slice, not yet on `main`
